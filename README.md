...
- **Three strategy families:**
  ...
  - **Ecdysis Engine EE-16:** evolved from EE-8 — a proprietary 16-indicator sentiment ensemble generating BUY/SELL/HOLD signals across 5 Solana assets (high-level only here).
...
> **Note:** SDK-based onboarding with OpenClaw/ClawKey will return post-alpha for agent developers. During the Private Alpha, Moltiverse is the primary interface.
...
- 5 Solana assets: ORCA, BONK, TRUMP, WBTC, WETH
...
**Access:** Currently **invite-only** via beta codes provided by the Sentry team. No deposit, no subscription, no trade fees during beta. Post-beta access will use a MOLTING token-gated model (details TBD). SSaaS still exists as API infrastructure, but Moltiverse is the primary interface now.
...
- **Market selection** — subscribers choose which of 5 tokens to receive signals for
...
- Portfolio value, total PnL (deposit-based), SOL balance, active positions, and recent trades
...
## EE-16 Private Alpha (LIVE)

The EE-16 engine is now **LIVE** in Private Alpha with a radically simplified onboarding process.
...
2. Enter an access code (provided to private alpha participants)
3. Set a 4-digit PIN
4. Receive a Solana wallet address and a unique Molty-Code (your user ID)
5. Deposit SOL → EE-16 starts trading automatically across all 5 markets

**No SDK, no API keys, no env files, no market selection.** The engine trades automatically across all 5 active markets (ORCA, BONK, TRUMP, WBTC, WETH). Users check their performance and withdraw via the same Moltiverse UI using their Molty-Code + PIN. Withdrawals close all positions and send SOL back to the user's wallet.

The dashboard displays: portfolio value, total PnL (deposit-based), SOL balance, active positions, and recent trades. Every trade is recorded and tracked for accurate PnL calculation. Stop-loss and trailing stop mechanisms are active.
...
> **Note:** The SDK-based agent onboarding flow (Steps 1–11 in [`docs/AGENT_ONBOARDING.md`](./docs/AGENT_ONBOARDING.md)) remains functional and will return as the primary onboarding path for OpenClaw agents after the Private Alpha concludes. During the alpha, Moltiverse is the recommended entry point for users. SDK-based onboarding with OpenClaw/ClawKey will return post-alpha for agent developers.
...
- ~~Accumulate candle history for high-confidence signals across all 5 markets~~ **DONE — EE-16 running**
...
- ~~Simplified human-first onboarding~~ **LIVE — Moltiverse Private Alpha is live**
- Add signal overlays and performance tracking to the Moltiverse dashboard
- SDK-based onboarding with OpenClaw/ClawKey for agent developers (post-alpha)
